...
| **Island graph and sleep/wake.** Node, edge, island managers and ordered change journals survive, along with flags that may skip an update when all are asleep ([PxsIslandManager.h:251](../../PhysX-3.3.3/PhysXSDK/Source/LowLevel/software/include/PxsIslandManager.h#L251), [PxsIslandManager.cpp:600](../../PhysX-3.3.3/PhysXSDK/Source/LowLevel/software/src/PxsIslandManager.cpp#L600)). Temporary work pointers are nulled after the pass ([PxsIslandManager.cpp:622](../../PhysX-3.3.3/PhysXSDK/Source/LowLevel/software/src/PxsIslandManager.cpp#L622)); graph and pool metadata remain. | **Observed system dependency** in the shipped trace (12 edges to 8 versus 0 to 0); **source dependency** in the offline fixture. | Oracle records graph/free lists/queues. `IslandImage` has idempotent same-topology restore ×100 and atomic rejection, but cannot resurrect an edge whose CM was destroyed ([IslandImage.h](../experiments/physx333-offline/island/IslandImage.h)). Restore NPhase/CM first; publish island graph last. Also capture `Sc::Scene` sleep/wake lists and `BodySim` flags. |
...
2. **Same-scene first-step gate.** Run `warm A -> delete B -> restore A ->
   ...
   settled suffix. The current component round trips do not satisfy this gate.
...
The shortest next implementation path is a joined read-only image, then a
same-scene NPhase/CM/contact-memory transaction, followed by body/scene and
island publication. The six-contact first-step gate must pass before expanding
the fixture matrix. No existing component test or fresh-scene replay proves
whole-scene rewind yet.
