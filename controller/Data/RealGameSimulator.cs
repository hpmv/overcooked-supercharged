...
namespace Hpmv
{
    public class RealGameSimulator
    {
        ...
        public void ApplyGameUpdate(Serialisable item)
        {
            ...
            Action<SpawnEntityMessage> spawnHandler = (SpawnEntityMessage sem) =>
            {
                ...
                if (spawner.prefab.IsStack)
                {
                    ...
                    {
                        // There's a special case, if we're spawning onto a clean plate stack and the top
                        // of the stack contains ingredients. In this case we insert the new plate below.
                        if ((d.stackContents?.Count ?? 0) >= 1 && d.stackContents[0].prefab.CanContainIngredients && (d.stackContents[0].data.Last().contents?.Count ?? 0) > 0)
                        {
                            d.stackContents = new List<GameEntityRecord>(d.stackContents);
                            d.stackContents.Insert(d.stackContents.Count - 1, child);
                            return d;
                        }
                ...
                else if (spawner.prefab.IsPlateReturnStation)
                {
                    // A plate return station automatically sets its spawn as its "stack".
                    spawner.data.AppendWith(frame, d =>
                    {
                        d.plateReturnStationStack = child;
                        return d;
                    });
                }
            };
        ...
        public void ApplyPhysicsPhaseShift(int shift)
        {
            setup.entityRecords.PhysicsPhaseShift.ChangeTo(shift, frame);
        }
