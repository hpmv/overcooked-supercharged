...
namespace Hpmv
{
    public static class SaveUtils
    {
        ...
        public static Save.VersionedString ToProto(this Versioned<string> ver)
        {
            var result = new Save.VersionedString { InitialValue = ver.initialValue };
            foreach (var (frame, value) in ver.changes)
            {
                result.Frame.Add(frame);
                result.Data.Add(value);
        ...
        public static Versioned<string> FromProto(this Save.VersionedString ver)
        {
            var result = new Versioned<string>(ver.InitialValue);
            for (int i = 0; i < ver.Frame.Count; i++)
            {
                result.ChangeTo(ver.Data[i], ver.Frame[i]);
            }
    ...
    public class LoadContext
    {
        ...
        public void Load(Save.GameEntityRecords records)
        {
            ...
            Records.InvalidStateReason = records.InvalidStateReason?.FromProto() ?? new("");
            Records.PhysicsPhaseShift = records.PhysicsPhaseShift?.FromProto() ?? new(0);
        }
    }
}