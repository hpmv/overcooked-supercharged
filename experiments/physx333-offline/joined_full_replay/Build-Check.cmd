...
set "PHYSX_LIB=%~dp0..\joined_topology\work\PhysXSDK\Lib\vc12win32\PhysX3_x86.lib"
if not exist "%VSDEVCMD%" exit /b 1
if not exist "%CMAKE%" exit /b 1
if not exist "%PHYSX_LIB%" (
  echo Build the isolated joined_topology source bridge first.
  exit /b 1
)
...
if errorlevel 1 exit /b 1
if not "%~1"=="" (
  "%~dp0out-ninja\physx333_joined_full_replay.exe" %*
  exit /b %errorlevel%
)
"%~dp0out-ninja\physx333_joined_full_replay.exe"
if errorlevel 1 exit /b 1
"%~dp0out-ninja\physx333_joined_full_replay.exe" --warm
if errorlevel 1 exit /b 1
"%~dp0out-ninja\physx333_joined_full_replay.exe" --shipped-order
if errorlevel 1 exit /b 1
"%~dp0out-ninja\physx333_joined_full_replay.exe" --shipped-order-warm
exit /b %errorlevel%
