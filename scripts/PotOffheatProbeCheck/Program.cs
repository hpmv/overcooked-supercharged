...
const string DwellJob = "P04-offheat-thirteen-seconds";
...
const string ParkKey = "counter:dlc08_countertop_01_standard_circus@19.20,-10.80";
...
bool Ready(JsonObject e, bool meal = false)
{
    ...
    return meal ? !observed.IsPlate && CarnivalRecipes.MatchRecipe(e, 296560, false).ReadyToDeliver :
        observed.EvidenceGaps.Length == 0 && observed.Food.Kind == FoodNodeKind.Cooked &&
        observed.Food.Preparation == FoodPreparation.Cooked && observed.Food.CookingStepId == CarnivalRecipes.PotCookingStepId &&
        observed.Food.IngredientIds.SequenceEqual(new[] { CarnivalRecipes.Frankfurter.Id });
}
JsonObject report;
try
{
    Require(args.Length == 4 || args.Length == 6 && args[4] == "--controller-bundle", "Usage: TRACE PLAN RESULT OUTPUT [--controller-bundle DIRECTORY]");
    string bundle = Path.GetFullPath(args.Length == 6 ? args[5] : "artifacts/planner-candidate-v11");
    ...
    Require(jobs.Length == 6 && jobs.Select(j => j["id"]!.ToString()).Distinct().Count() == 6 && I(plan["timeoutFrames"]) == 3600, "Unexpected authored probe bounds.");
    ...
    int pot = 0, home = 0, park = 0, output = 0, board = 0, food = 0, potOrdinal = -1, homeOrdinal = -1, parkOrdinal = -1, foodOrdinal = -1;
    ...
    bool dwelling = false, dwellDone = false, heldCooked = false, heldCombined = false;
    ...
    void Dwell(JsonObject s)
    {
        ...
        var b = Entity(s, pot); var h = Entity(s, home); var p = Entity(s, park);
        ...
        Require(I(h["attachedEntityId"]) == 0 && I(p["attachedEntityId"]) == pot && Ready(b) &&
            s["chefs"]!.AsArray().OfType<JsonObject>().All(c => I(c["heldEntityId"]) != pot), "Dwell pot is not the same fully cooked plain hotdog recipe on its ordinary offheat counter.");
    ...
    while ((line = reader.ReadLine()) is not null)
    {
        ...
        if (pot == 0)
        {
            ...
            pot = model.Resolve(PotKey).EntityId; home = model.Resolve(HomeKey).EntityId; park = model.Resolve(ParkKey).EntityId;
            output = model.Resolve(OutputKey).EntityId; board = model.Resolve(BoardKey).EntityId;
            potOrdinal = I(Entity(initial, pot)["observedOrdinal"]); homeOrdinal = I(Entity(initial, home)["observedOrdinal"]);
            parkOrdinal = I(Entity(initial, park)["observedOrdinal"]); 
            Require(I(Entity(initial, home)["attachedEntityId"]) == pot && I(Entity(initial, park)["attachedEntityId"]) == 0 &&
                CarnivalRecipes.ClassifyEntity(Entity(initial, pot)).Food.IngredientIds.Length == 0, "Initial exact pot/stove or empty parking state differs.");
            firstFrame = frameNow;
        }
        ...
        var selected = native.GetProperty("entities").EnumerateArray().Where(e => new[] { pot, home, park, output, board, food }.Contains(e.GetProperty("id").GetInt32()));
        ...
        Require(I(vessel["observedOrdinal"]) == potOrdinal && I(Entity(s, home)["observedOrdinal"]) == homeOrdinal &&
            ...
            "Native pot, original stove, storage identity or cooking duration changed outside dwell.");
        ...
        if (I(Entity(s, home)["attachedEntityId"]) == pot && CarnivalRecipes.ClassifyEntity(vessel).Food.IngredientIds.Length > 0)
        ...
        if (I(chef["heldEntityId"]) == pot && N(chef["dashTimer"]) > 0) heldVesselDashFrames++;
        if (I(chef["heldEntityId"]) == pot && I(Entity(s, home)["attachedEntityId"]) == 0 && Ready(vessel)) heldCooked = true;
        if (dwellDone && I(chef["heldEntityId"]) == food && Ready(Entity(s, food), true)) heldCombined = true;
        if (dwelling) Dwell(s);
        last = s; stateSamples++;
    ...
    Require(heldCooked && maxHeat - minHeat >= 11.9 && heldCombined, "Native full heating, whole-pot pickup or cooked combined plain hotdog transition missing.");
    Require(last is not null && lastFrame == I(resultState["gameplayFrame"]), "Final result is not the last recorded frame.");
    Require(I(Entity(last!, output)["attachedEntityId"]) == food && I(Entity(last!, food)["observedOrdinal"]) == foodOrdinal && Ready(Entity(last!, food), true), "Final exact food lacks native completed plain hotdog recipe at output.");
    Require(I(Entity(last!, home)["attachedEntityId"]) == pot && I(Entity(last!, park)["attachedEntityId"]) == 0 &&
        CarnivalRecipes.ClassifyEntity(Entity(last!, pot)).Food.IngredientIds.Length == 0 && N(Entity(last!, pot)["cookingProgress"]) == 0, "Original empty pot not restored/reset on its original stove.");
    ...
    report = new() { ["ok"] = true, ["classification"] = "Observed native pot offheat/combine/restore probe; no high-score or replay qualification.",
        ["tracePath"] = Path.GetFullPath(args[0]), ["traceBytes"] = traceBytes, ["traceSha256"] = traceHash,
        ["planSha256"] = HashFile(args[1]), ["resultSha256"] = HashFile(args[2]), ["checkerSourceSha256"] = HashFile("scripts/PotOffheatProbeCheck/Program.cs"),
        ["checkerProjectSha256"] = HashFile("scripts/PotOffheatProbeCheck/PotOffheatProbeCheck.csproj"), ["controllerSha256"] = controllerHash,
        ...
        ["controllerSelection"] = args.Length == 6 ? "explicit --controller-bundle" : "legacy V11 default",
        ...
        ["potId"] = pot, ["potObservedOrdinal"] = potOrdinal, ["stoveId"] = home, ["stoveObservedOrdinal"] = homeOrdinal,
        ...
        ["wholeCookedPotPickupObserved"] = heldCooked, ["nativeCookedCombinedPlainHotdogObserved"] = heldCombined,
        ["finalRecipeId"] = 296560, ["originalEmptyPotRestored"] = true, ["choppedBunBoard"] = board, ["sameObservedChoppedBunBecameHotdog"] = true, ["fourControlledEmptyChefs"] = true,
