...
try
{
    ...
    string candidateHash = Convert.ToHexString(SHA256.HashData(File.ReadAllBytes("artifacts/planner-candidate-v16/OvercookedTAS.Controller.dll")));
    if (assemblyHash != candidateHash) throw new InvalidOperationException("Harness is not testing the frozen V16 binary.");
