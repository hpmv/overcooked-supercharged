...
try
{
    ...
    string candidateHash = Convert.ToHexString(SHA256.HashData(File.ReadAllBytes("artifacts/planner-candidate-v23/OvercookedTAS.Controller.dll")));
    if (assemblyHash != candidateHash) throw new InvalidOperationException("Harness is not testing the frozen V23 binary.");
    ...
    Passed("Conditional far pot planner admission", CarnivalPlanner.ConditionalFarPotSelfTest(Load("empty-lane-far-pot-prearm-state")));
    Passed("Far native interception", CarnivalPlanner.FarInterceptionSelfTest(Load("v18-failure-projection/gf4120"), Load("v18-failure-projection/gf4130"), Load("v18-failure-projection/gf4138")));
    Passed("Final native sauce head wait", CarnivalPlanner.FinalSauceWaitSelfTest(Load("v16-service-departure-gf11099"), Load("v16-final-sauce-gf11104"), Load("v16-final-sauce-gf11105"), Load("v16-final-sauce-gf11184"), Load("v16-service-head-complete-gf11185"), Load("v16-service-departure-gf12407")));
    Passed("Predictive one-kit bakery return", CarnivalPlanner.PredictiveBakerySelfTest(Load("v16-supply-snapshots/gf12373"), Load("v16-supply-snapshots/gf11069"), Load("v16-supply-snapshots/gf11445"), Load("v16-supply-snapshots/gf14401"), Load("v16-supply-snapshots/gf15625"), Load("native-round-v16/native-preview-call")));
    Passed("Fixed sausage transfer budget", CarnivalPlanner.SausageTransferBudgetSelfTest(Load("v18-buffer1-gf2497"), Load("v18-buffer1-gf2738")));
    IEnumerable<JsonObject> TransferResponses()
    {
    ...
    Passed("Recorded native sausage transfer samples", CarnivalPlanner.SausageTransferNativeTraceSelfTest(TransferResponses()));
    Passed("Single clean plate preserves earlier eligible meal", CarnivalPlanner.DirectCleanFairnessSelfTest(Load("v17-head-plate-gf14995"), Load("native-round-v17/native-preview-call")));
    Passed("One-plate onion finishing transaction", CarnivalPlanner.PlatedOnionSelfTest(Load("v16-onion-chain-gf5730")));
    Passed("FIFO assignment of original empty bowls", CarnivalPlanner.FifoEmptyBowlsSelfTest(Load("v19-bakery-priority-gf13165"), Load("v19-bakery-priority-gf11738"), Load("v19-bakery-priority-gf12355"), Load("v19-bakery-priority-gf13247"), Load("native-round-v19/native-preview-call"), JsonNode.Parse(File.ReadAllText("artifacts/v19-late-bakery-events.json"))!.AsArray()));
    Passed("One-plate hotdog base production", CarnivalPlanner.PlatedHotdogBaseSelfTest(Load("v16-plate-first-opportunities/gf789"), Load("v16-plate-first-opportunities/gf6377"), Load("native-round-v16/native-preview-call")));
    int[] stationaryFrames = [435,436,437,438,444,464,465,471,472,473,474,785,786,792,1075,1076,1082,1156,1157,1163];
    var stationaryCaptured = stationaryFrames.ToDictionary(f => f, f => Load((f is 435 or 436 ? "early-transfer-b-gf" : "stationary-transfer-b-gf") + f));
    Passed("Stationary exact native target transfer", RouteRunner.StationaryTransferSelfTest(stationaryCaptured));
    Passed("Optional stationary transfer planner defaults", RouteRunner.StationaryTransferDefaultsSelfTest(Load("stationary-transfer-b-gf437"), Load("stationary-transfer-b-gf438")));
