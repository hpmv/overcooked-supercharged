...
{
    // Adapted from plugin/JsonText.cs. Unity 2017's serializer omits several nested plugin DTOs. Serialize the
    ...
    internal static class JsonText
    {
        ...
        private static void Write(StringBuilder b,object value,int depth)
        {
            ...
            if (number != null) { b.Append(number.Token); return; }
            ...
            if (dictionary != null)
            {
                ...
                {
                    if (!(item.Key is string)) throw new InvalidOperationException("JSON object keys must be strings.");
                    if (!first) b.Append(','); first = false;
                    Quoted(b, (string)item.Key); b.Append(':'); Write(b,item.Value,depth+1);
                }
                b.Append('}'); return;
            }
